...
`xud` uses [gRPC](https://grpc.io/). The latest overview of available calls with descriptions can be found in [here](http://api.exchangeunion.com).
...
* `SubscribeOrders` this streaming call subscribes to orders being added to and removed from the order book. This allows the client to maintain an up-to-date view of the order book. Use this call to show a real-time order book to users via your exchange front-end and update account balances for your users.
...
* `SubscribeOrders` informs about new and removed `peerOrder`s in an event stream. Use this call to include these `peerOrder`s in your internal order book & matching engine. Adjust your system to call `ExecuteSwap`, if a match is found for a specific `peerOrder`.
...
* To query existing orders one can use `ListOrders`, in addition to the `existing` flag for the `SubscribeOrders` streaming call.
